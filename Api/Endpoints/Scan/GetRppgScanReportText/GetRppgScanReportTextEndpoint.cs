...
/// Вся логика интерпретации и форматирования сосредоточена в этом классе.
/// </summary>
sealed class GetRppgScanReportTextEndpoint : Endpoint<GetRppgScanReportTextRequest>
{
    ...
    public GetRppgScanReportTextEndpoint(AppDbContext db)
    {
        _db = db;
    }
    ...
    public override async Task HandleAsync(GetRppgScanReportTextRequest req, CancellationToken ct)
    {
        var scan = await _db.UserRppgScans
            ...
            .FirstOrDefaultAsync(s => s.Id == req.ScanId, ct);
        ...
        var report = BuildReport(scan, profile, age, genderInt, weight, biomarkers);
